...
Features 100% on-device offline wake-word detection, full-duplex WebSocket streaming, an animated OLED facial expression engine (LVGL v9), and a custom J.A.R.V.I.S. British AI persona with multi-LLM backend support (DeepSeek V4 / GPT-5 / Qwen) and RAG.
...
- **Speaker:** 50mm Hi-Fi 4Ω/8Ω Acoustic Transducer
...
2. **Dual-Core FreeRTOS Task Pinning:**  
   Separated processing pipelines to eliminate audio starvation:
...
4. **Power Rail Isolation & Acoustic Physics:**  
   Separated sensitive logic (ESP32/Mic on 3.3V LDO) from the high-transient 3W amplifier (connected to 5V VBUS with 15dB hardware gain) to prevent brownout resets during Wi-Fi transmissions and audio peaks.
